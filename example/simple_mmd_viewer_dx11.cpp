...
void Model::Draw(const AppContext& appContext)
{
	...
	for (size_t i = 0; i < subMeshCount; i++)
	{
		...
		m_context->PSSetShader(appContext.m_mmdGroundShadowPS.Get(), nullptr, 0);
		{
			...
			psCB.m_shadowColor = glm::vec4(0.0f, 0.0f, 0.0f, 0.7f);
...
int main(int argc, char** argv)
{
	...
	std::vector<std::string> args(argc);
	{
		...
		const WCHAR* cmdline = L"-model S:/Code/saba/build/example/ltyv4/luotianyi_v4_ver2.0.pmx";
		int wArgc;
	...
	//SetLayeredWindowAttributes(hwnd,RGB(255,255,255),(BYTE)255,LWA_ALPHA|LWA_COLORKEY);
