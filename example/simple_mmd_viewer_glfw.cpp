...
//	int wingsIdx = 0;
//	double wingsWeight = 0.0;
//};

struct MMDShader
{
	~MMDShader() // 析构函数，在开始和结束时候会自动触发，相当于初始化
	{
		Clear();  //下文有定义
	}

	GLuint	m_prog = 0; //OpenGL的int类型
...
struct AppContext
{
	...
	std::unique_ptr<MMDShader>				m_mmdShader;  //智能指针：行为像常规指针，但智能指针能够及时、妥善地销毁动态分配的数据，并实现了明确的对象生命周期
...
struct Material
{
	...
	/*具有explicit修饰的构造函数被称为显式构造函数。它的作用是防止隐式转换发生：通常情况下，当我们声明一个构造函数时，可以用它来进行隐式类型转换。
	例如，如果我们有一个Material对象m，我们可以使用一个saba::MMDMaterial对象mat来初始化它，即m = mat; 这个操作会隐式地调用Material的构造函数。
	但是，加上explicit修饰后，只能通过显式调用构造函数来生成一个Material对象。也就是说，必须使用Material对象的构造函数来显式地将saba::MMDMaterial对象转换为Material对象，
	而不能通过隐式转换来进行。这样可以防止在代码中产生一些不易察觉的错误。*/
...
struct Model
{
	...
	GLuint	m_mmdwingsIdx = 0;
	GLdouble m_mmdwingsweight = 1.0;

	GLuint	m_mmdeyesIdx = 0;
	GLdouble m_mmdeyesweight = 0.0;

	double blink_time = 4.0, model_time = saba::GetTime();
...
rapidjson::Document readJsonFile(const std::string& filePath)
{
	...
	if (!file.is_open()) {
		std::cerr << "无法打开JSON文件：" << filePath << std::endl;
		return document;
	}
	...
	try {
		...
		// 解析JSON字符串为rapidjson的Document对象
		document.Parse(jsonString.c_str());

		// 检查解析是否成功
		if (document.HasParseError()) {
			std::cerr << "解析JSON文件失败。错误码: " << document.GetParseError() << std::endl;
			document.SetObject();  // 解析失败时返回一个空的Document对象
		}
	}
	catch (...) {
		std::cerr << "读取JSON文件时发生异常。" << std::endl;
		document.SetObject();  // 发生异常时返回一个空的Document对象
	}
...
bool Model::Setup(AppContext& appContext)
{
	...
	auto wings_morph = morphMan->GetMorph(m_mmdwingsIdx);
	auto eyes_morph = morphMan->GetMorph(m_mmdeyesIdx);

	wings_morph->SetWeight(m_mmdwingsweight);
	eyes_morph->SetWeight(m_mmdeyesweight);
...
void Model::Update(const AppContext& appContext)
{
	...
	//outfile.open("C:\\Users\\z2538\\Desktop\\morpidx.txt", std::ios::out | std::ios::app); // open file in write append mode

	//for (size_t morphIdx = 0; morphIdx < morphCount; morphIdx++)
	//{
	//	auto morph = morphMan->GetMorph(morphIdx);
	//	float weight = morph->GetWeight();
	//	outfile << morphIdx <<' '<< morph->GetName().c_str() << std::endl;
	//}
...
void Model::UpdateBlink(double n_time)
{
	double t_delta = n_time - model_time;
	if (std::abs(t_delta - 0.1) < 0.03)
	{
		m_mmdeyesweight = 1.0;
	}
	else
	if (t_delta < 0.1)
	{
		m_mmdeyesweight = 0.5*std::sin(65.0*(t_delta-1.5707963))+0.5;
	}
	else
	if (t_delta < 0.3)
	{
		m_mmdeyesweight = 1166.0 / 3.0 * std::pow(0.3 - t_delta, 3) * t_delta;
	}
	else
	m_mmdeyesweight = 0.0;
	...
	auto morphMan = m_mmdModel->GetMorphManager();
	size_t morphCount = morphMan->GetMorphCount();
	auto morph = morphMan->GetMorph(m_mmdeyesIdx);
	morph->SetWeight(m_mmdeyesweight);

	if (t_delta > blink_time)
	{
		std::default_random_engine rand_gen(std::time(0));
		std::uniform_real_distribution<double> r_value(0.5, 8.0);
		blink_time = r_value(rand_gen);
		model_time = n_time;
	}
}
...
void Model::Draw(const AppContext& appContext)
{
	...
	auto shadowColor = glm::vec4(0.0f, 0.0f, 0.0f, 0.7f);
	if (appContext.m_enableTransparentWindow)
	{
		glEnable(GL_BLEND);
		glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE_MINUS_SRC_ALPHA);
...
bool SampleMain(const rapidjson::Document& basic)
{
	
	std::string w_title = "";
	int w_width = 480, w_height = 640;
	double w_posx = 0.0, w_posy = 0.0;
	bool enableTransparentWindow = false;

	assert(basic.HasMember("windows"));

	w_title = basic["windows"]["title"].GetString();
	w_width = basic["windows"]["width"].GetInt();
	w_height = basic["windows"]["height"].GetInt();
	w_posx = basic["windows"]["posx"].GetDouble();
	w_posy = basic["windows"]["posy"].GetDouble();

	if (basic["windows"]["transparent"] == "true")
	...
	auto window = glfwCreateWindow(w_width, w_height, w_title.c_str(), nullptr, nullptr);
	glfwSetWindowAttrib(window, GLFW_FLOATING, GLFW_TRUE);
	glfwSetWindowPos(window, w_posx, w_posy);
	if (window == nullptr)
	{
		return false;
	}
	...
	// Load MMD model
	assert(basic.HasMember("models"));

	std::vector<Model> models;
	for (auto& m_modeler : basic["models"].GetArray())
	{
		// Load MMD model
		Model model;
		auto ext = saba::PathUtil::GetExt(m_modeler["model_path"].GetString());
		if (ext == "pmd")
		{
			auto pmdModel = std::make_unique<saba::PMDModel>();
			if (!pmdModel->Load(m_modeler["model_path"].GetString(), appContext.m_mmdDir))
			{
		...
		else if (ext == "pmx")
		{
			...
			if (!pmxModel->Load(m_modeler["model_path"].GetString(), appContext.m_mmdDir))
			{
		...
		if (m_modeler.HasMember("wings"))
		{
			model.m_mmdwingsIdx = m_modeler["wings"]["wingsIdx"].GetInt();
			model.m_mmdwingsweight = m_modeler["wings"]["wingsWeight"].GetDouble();
		}
		if (m_modeler.HasMember("eyes"))
		{
			model.m_mmdeyesIdx = m_modeler["eyes"]["eyesIdx"].GetInt();
			model.m_mmdeyesweight = m_modeler["eyes"]["eyesWeight"].GetDouble();
		}
...
{
	const std::string filePath = "S:/Code/saba-master/settings/basic.json";

	rapidjson::Document basic = readJsonFile(filePath);
